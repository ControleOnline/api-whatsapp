...
const baileysMessageListeners = (wbot, telefone) => {
  ...
  wbot.ev.on('messaging-history.set', async (data) => {
    ...
    const webhookUrl = fetchWebHook(wbot, 'messaging-history.set')
    if (!webhookUrl) return

    await sendWebhook(webhookUrl, { messages: JSON.stringify(conversas) })
  })
  ...
  wbot.ev.on('messages.upsert', async (messageUpsert) => {
    ...
    const webhookUrl = fetchWebHook(wbot, 'messages.upsert')
  ...
  wbot.ev.on('messages.update', async (messageUpdate) => {
    ...
    const webhookUrl = fetchWebHook(wbot, 'messages.update')
    if (!webhookUrl) return
